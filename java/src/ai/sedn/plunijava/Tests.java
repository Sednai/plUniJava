...
public class Tests {
	...
	public static Iterator test_njdbc3() throws Throwable {
		
		ArrayList<TestType4> L = new ArrayList<TestType4>();
		
		PlUniJava unij = new PlUniJava();
			
	    unij.connect();
	    
	    unij.execute("select id,data from test_table3 order by id");
	    
	    while(unij.fetch_next()) {
			TestType4 R = new TestType4();
	    	R.A = unij.getstring(2);
	...
	public static Iterator test_njdbc4() throws Throwable {
	    ...
	    unij.execute("select id,data from test_table4 order by id");
	    
	    while(unij.fetch_next()) {
			TestType1 R = new TestType1();
	    	R.A = unij.getintarray(2)[0];
			R.B = 0;
	...
	public static Iterator test_njdbc5() throws Throwable {
	    ...
	    unij.execute("select id,data from test_table5 order by id");
	    
	    while(unij.fetch_next()) {
			TestType5 R = new TestType5();
	    	R.A = unij.getint(1);
			R.B = unij.getfloatarray(2)[0];
			L.add(R);
	    }
